...
export function WorkoutDisplay({ onBack }: { onBack: () => void }) {
  ...
  const getExerciseTime = (stationIndex: number): number => {
  ...
  const getInitialState = (): TimerState => ({
    ...
    isRunning: false,
    totalRounds: currentWorkout.rounds,
  });
    ...
    setState(prev => {
      ...
      if (prev.timeRemaining <= 1) {
        ...
        if (prev.phase === 'exercise') {
          ...
          if (nextStation >= currentWorkout.stations.length) {
            if (prev.currentRound >= prev.totalRounds) {
              ...
              return {
                ...prev,
                phase: 'rest-round',
                timeRemaining: currentWorkout.restBetweenRounds,
              };
            }
          } else {
            return {
              ...prev,
              phase: 'rest-station',
              timeRemaining: currentWorkout.restBetweenStations,
            };
          }
  ...
  }, [playBeep, playLongBeep, currentWorkout.stations.length, currentWorkout.restBetweenStations, currentWorkout.restBetweenRounds]);
  ...
  const currentStation = currentWorkout.stations[state.currentStation];
  const exerciseTime = getExerciseTime(state.currentStation);

  const progress = state.phase === 'exercise'
    ? 1 - (state.timeRemaining / exerciseTime)
    : state.phase === 'rest-station' || state.phase === 'rest-round'
    ? 1 - (state.timeRemaining / (state.phase === 'rest-station' ? currentWorkout.restBetweenStations : currentWorkout.restBetweenRounds))
    : 1;
  ...
  const getPhaseSublabel = () => {
    switch (state.phase) {
      ...
      case 'rest-station':
        return `Next: ${currentWorkout.stations[state.currentStation + 1]?.exercise?.name || 'Exercise'}`;
      case 'rest-round':
      ...
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
        <div className="text-center fade-in">
          ...
          <p className="text-xl text-zinc-400 mb-8">
            You completed {currentWorkout.name} with {currentWorkout.rounds} rounds!
          </p>
          <div className="flex gap-4 justify-center flex-col sm:flex-row">
            ...
            <button
              onClick={onBack}
              className="flex items-center justify-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-medium transition-colors"
            >
              Back to Editor
            </button>
          </div>
      ...
      <header className="bg-zinc-900 border-b border-zinc-800 p-4">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="font-semibold text-white">{currentWorkout.name}</h1>
            <p className="text-xs text-zinc-400">Round {state.currentRound} of {currentWorkout.rounds}</p>
          </div>
          <button
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="text-center max-w-2xl w-full">
          ...
          {state.phase === 'exercise' && currentStation?.exercise && (
            <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-4 mb-8 fade-in">
